...
async function analyzeKeyword(kw, ytKey, { days = 90, region = "US" } = {}) {
  const publishedAfter = new Date(Date.now() - days * 864e5).toISOString();
  const s = await ytApi("search", { part: "snippet", q: kw, type: "video", order: "viewCount", publishedAfter, maxResults: 25, regionCode: region, relevanceLanguage: "en" }, ytKey);
  const ids = (s.items || []).map(i => i.id.videoId).filter(Boolean);
  if (!ids.length) throw new Error("No recent videos found for this keyword");
  const vd = await ytApi("videos", { part: "snippet,statistics,contentDetails", id: ids.join(",") }, ytKey);
  ...
  return { kw, score, demand: Math.round(demand), opportunity: Math.round(opportunity), velocity: Math.round(velocity), competition: Math.round(competition), engagement, avgViews, medViews, avgVpd, channels: chList.length, smallWinners: smallWinners.length, outliers, breakout, vids: vids.slice(0, 6) };
}
...
export default function NicheFinder({ ytKey, clKey, niches, sn, goNiche }) {
  ...
  const suggest = async () => {
    if (!clKey) { setSt("⚠️ Add your Anthropic key in Settings for AI suggestions"); return; }
    if (!seed.trim()) { setSt("⚠️ Type a broad topic first, e.g. “history”, “finance”, “true crime”"); return; }
  ...
  const analyze = async () => {
    if (!ytKey) { setSt("⚠️ Add your YouTube API key in Settings"); return; }
    if (!kws.length) { setSt("⚠️ Add at least one keyword"); return; }
    cancelRef.current = false; setBusy(true);
    const done = {};
    for (let i = 0; i < kws.length; i++) {
      if (cancelRef.current) break;
      const kw = kws[i];
      if (results[kw]?.data) { done[kw] = results[kw]; continue; }
      setSt(`📡 Analyzing ${i + 1}/${kws.length}: “${kw}”...`);
      setResults(prev => ({ ...prev, [kw]: { status: "loading" } }));
      try {
        const data = await analyzeKeyword(kw, ytKey, { days, region });
  ...
  const saveNiche = (r, go) => {
    ...
    if (!n) {
      n = { id: Date.now(), name, desc: `Niche Finder — score ${r.score}/100 · avg ${fmt(r.avgViews)} views · ${r.smallWinners} small channels winning`, channels: r.breakout.slice(0, 4).map(c => c.id), history: [], finder: { score: r.score, date: new Date().toISOString().slice(0, 10) } };
      sn([...niches, n]);
    }
    if (go) goNiche(n);
  };
  ...
  return (<div className="yt-page">
    ...
    {kws.filter(k => results[k]).map(kw => {
      ...
      if (r.status === "error") return <div key={kw} className="yt-card nf-card"><p className="yt-st err">⚠️ {kw}: {r.err}</p></div>;
      const d = r.data;
      return (<div key={kw} className="yt-card nf-card">
        <div className="nf-head">
          <div className="nf-head-l">
            <h3 className="nf-kw">{titleCase(d.kw)}</h3>
...
.nf-chip-score{background:var(--green);color:#04150d;font-size:10px;padding:1px 7px;border-radius:8px;margin-left:2px}
...
.nf-kw{font-size:20px;font-weight:700;letter-spacing:-.3px;margin-bottom:6px}
.nf-verdict{font-size:12.5px;color:var(--text2);line-height:1.5;margin-bottom:10px;background:var(--surface);border-left:3px solid var(--red);padding:8px 12px;border-radius:0 8px 8px 0}
...
.nf-bar-fill{height:100%;background:linear-gradient(90deg,#34d399,#4d9fff);border-radius:4px;transition:width 1s cubic-bezier(.16,1,.3,1)}
.nf-bar-fill.bad{background:linear-gradient(90deg,#f59e0b,#ff3b3b)}
...
.nf-gauge.hot .nf-gauge-num{color:#34d399}.nf-gauge.warm .nf-gauge-num{color:#f59e0b}.nf-gauge.cold .nf-gauge-num{color:var(--text3)}
...
.nf-gauge-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,#ff3b3b,#f59e0b,#34d399)}
...
.nf-vid:hover{border-color:var(--red);transform:translateY(-2px)}
